...
class DivergenceFunction(Theanifiable):
    ...
    @theanify(T.dscalar('s'), T.dscalar('t'), T.dscalar('m'), T.dscalar('c'))
    def no_divergence(self, s, t, m, c):
        return T.exp(self.log_no_divergence(s, t, m, c))
...
class Inverse(DivergenceFunction):
    ...
    @theanify(T.dscalar('s'), T.dscalar('t'), T.dscalar('m'), T.dscalar('c'))
    def log_no_divergence(self, s, t, m, c):
        # res = 0
        # for i in range(1, m):
        #    res += 1.0/i
